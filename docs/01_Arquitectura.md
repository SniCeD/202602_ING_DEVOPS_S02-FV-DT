# Cuentas — Arquitectura del microservicio

## Componentes

| Componente | Rol |
|---|---|
| `CuentaApplication` | Punto de entrada Spring Boot |
| `controller/CuentaController.java` | API REST expuesta en `/api/cuentas` |
| `service/CuentaService.java` | Lógica de negocio y transacciones |
| `repository/CuentaRepository.java` | Acceso JPA a H2 |
| `model/Cuenta.java` | Entidad persistida en tablas H2 |
...
`Cliente/Sistema → API Gateway → CuentaController → CuentaService → CuentaRepository → H2`
