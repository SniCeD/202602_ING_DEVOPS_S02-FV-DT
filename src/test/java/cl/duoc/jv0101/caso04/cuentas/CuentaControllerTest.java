...
class CuentaControllerTest {
    ...
    private Cuenta recurso(Long id) {
        Cuenta r = new Cuenta();
        r.setId(id);
        r.setNombre("Demo");
        r.setTitular("valor");
    ...
    void obtenerInexistenteDevuelve404() throws Exception {
        ...
        mockMvc.perform(get("/api/cuentas/1")).andExpect(status().isNotFound());
    }
    ...
    void eliminarExistenteDevuelve204() throws Exception {
        ...
        mockMvc.perform(delete("/api/cuentas/1")).andExpect(status().isNoContent());
    }

    @Test
    void eliminarInexistenteDevuelve404() throws Exception {
        when(service.delete(1L)).thenReturn(false);
        mockMvc.perform(delete("/api/cuentas/1")).andExpect(status().isNotFound());
    }
}
