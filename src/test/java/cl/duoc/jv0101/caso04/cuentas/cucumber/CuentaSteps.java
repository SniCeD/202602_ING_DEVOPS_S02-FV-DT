package cl.duoc.jv0101.caso04.cuentas.cucumber;
...
public class CuentaSteps {
    ...
    private Long id;
    private ResponseEntity<Cuenta> respuesta;
    private ResponseEntity<Void> respuestaVoid;

    private String url() {
        return "http://localhost:" + port + "/api/cuentas";
    }
    ...
    public void servicioDisponible(String servicio) {
        ResponseEntity<Cuenta[]> listado = rest.getForEntity(url(), Cuenta[].class);
    ...
    public void crearRecurso(String recurso, String nombre) {
        ResponseEntity<Cuenta> creado = rest.postForEntity(
                url(), body(nombre), Cuenta.class);
    ...
    public void consultarCreado(String recurso) {
        respuesta = rest.getForEntity(url() + "/" + id, Cuenta.class);
    }
    ...
    public void actualizar(String recurso, String nombre) {
        respuesta = rest.exchange(url() + "/" + id, HttpMethod.PUT, body(nombre), Cuenta.class);
    }
    ...
    public void consultarEliminado(String recurso, int codigo) {
        ResponseEntity<Cuenta> noExiste = rest.getForEntity(url() + "/" + id, Cuenta.class);
